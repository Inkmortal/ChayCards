...
/**
 * Repository for handling flashcard persistence
 */
export class FlashcardRepository {
  ...
  async createFlashcard(data: Partial<FlashcardDocument>): Promise<FlashcardDocument> {
    ...
    return db.transaction(() => {
      try {
        // First create the base document
        db.run(`
        ...
        // Initialize spaced repetition data if not provided
        ...
        // Then create the flashcard-specific data
        db.run(`
  ...
  async deleteFlashcard(id: string): Promise<void> {
    const db = getDatabase();
    // Due to CASCADE, deleting from documents will also delete from flashcards
  ...
  /**
   * Map database flashcard to domain model
   */
